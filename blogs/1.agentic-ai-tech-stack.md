...
This document explains the "Agentic AI Tech Stack" landscape diagram. The stack is
organized roughly top-to-bottom from **what the user sees** (applications) down to
**where the model actually runs** (inference hardware). Two vertical sidebars cut
across all layers: **Observability/Monitoring/Evals** and **AI Safety/Security**,
because both concerns apply at every level of the stack.
...
[[2]](https://www.mindstudio.ai/blog/ai-coding-agent-harness-stripe-shopify-airbnb)

The diagram splits harnesses into **Personal Agents** (general/24-7 assistants) and
**Coding Agents** (specialized for editing and running code).

| Tool | Category | What it is | Source |
|------|----------|------------|--------|
| **Gemini Spark** | Personal Agent | Google's 24/7 personal AI agent (announced at Google I/O 2026). Persists between sessions, runs in the background on cloud infrastructure, and takes actions across tools. Notably "built from Gemini base models and an **agentic harness** from Google Antigravity," with native Gmail/Calendar/Drive/Docs integration. | [Google](https://gemini.google/overview/agent/spark/) · [TechCrunch](https://techcrunch.com/2026/05/19/google-introduces-gemini-spark-a-24-7-agentic-assistant-with-gmail-integration/) |
| **Claude Code** | Coding Agent | Anthropic's agentic coding tool that lives in your terminal, understands your codebase, and executes tasks (edit files, run commands, handle git) via natural language. Its underlying harness — the **Claude Agent SDK** (formerly Claude Code SDK) — can power general-purpose agents too. | [Anthropic](https://www.anthropic.com/product/claude-code) · [Docs](https://code.claude.com/docs/en/overview) · [GitHub](https://github.com/anthropics/claude-code) |
| **Cursor** | Coding Agent | An AI-native IDE (VS Code fork) and coding agent. Its agents search the codebase, edit files, run terminal commands, and complete multi-step tasks from natural language; Cursor 3 can run up to 8 agents in parallel on isolated git branches. | [Cursor](https://cursor.com/) · [Wikipedia](https://en.wikipedia.org/wiki/Cursor_(code_editor)) |

> **Note on unlabeled icons:** The "Personal Agent" subsection of the diagram also
> shows 2–3 logos (a red robot, a blue terminal-style mark, and a lightning/flash
> icon) that are too ambiguous to identify with confidence from the image alone. They
> are intentionally omitted from the table rather than guessed — if you can confirm
> which products they are, they belong in the Personal Agent rows.
...
- **Pydantic (PydanticAI)** — type-safe, structured-output agent framework.
...
- **Closed weights:** **Gemini** (Google), **OpenAI**, **Anthropic**, **Meta (Llama – select)**, **Mistral**.
...
- **Observability and security are not layers but dimensions** — you need them at
  every level, which is why they span the full height of the diagram.
